 ...
 * @internal
 * @var int cleared
 * @brief If it is 0 the log file will be cleared, if is 1 the log file will we
 * open as append
 * */
int cleared = 0;
 ...
 * @note \"Err_Numebr:%d\" will be appended to the formatted string describing
 * the error number
 * @note after Err_Number \"-> %s\" will be appended printing the std-error
 * */
void
print_err (const char *format, ...)
{
  va_list args;
  ...
  sd_journal_print (LOG_ERR, "ERROR=%s Err_Number:%d -> %s", buffer, errno,
                    strerror (errno));
}
/**
