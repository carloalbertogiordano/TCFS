...
/**
 * @file json_tools.c
 * @brief This file provides function to cast either qm_user, qm_shared or qm_broad to a json string and vice versa
 * @note All the functions here are C++ functions, so we could use nlohmann-json library
 * @note Learn more on [nlohmann-json](https://github.com/nlohmann/json)
 * */

/**
 * @brief Cast a qm_user, qm_shared or qm_broad struct to a json string representing the struct
 ...
 * @param const char * json_string      The string containing the json that represents the struct
 ...
 * @note To cast the returned param to the structure you probably need to use a switch(type) and cast it to a struct
 * @see common.h
 * */
void* string_to_struct(const char* json_string, qm_type* type) {
    try {
        ...
        if (json_obj.contains("user_op")) {
            *type = USER;
            qm_user* user = static_cast<qm_user*>(std::malloc(sizeof(qm_user)));
            user->user_op = json_obj["user_op"];
            user->pid = json_obj["pid"];
            user->user = strdup(json_obj["user"].get<std::string>().c_str());
            user->pubkey = strdup(json_obj["pubkey"].get<std::string>().c_str());
            return user;
        } else if (json_obj.contains("fd")) {
            *type = SHARED;
            qm_shared* shared = static_cast<qm_shared*>(std::malloc(sizeof(qm_shared)));
            shared->fd = json_obj["fd"];

            // Populate userlist array
            std::vector<std::string> userlist = json_obj["userlist"];
            shared->userlist = static_cast<char**>(std::malloc((userlist.size() + 1) * sizeof(char*)));
            for (size_t i = 0; i < userlist.size(); ++i) {
                shared->userlist[i] = strdup(userlist[i].c_str());
            }
            shared->userlist[userlist.size()] = nullptr;

            shared->keypart = strdup(json_obj["keypart"].get<std::string>().c_str());
            return shared;
        } else if (json_obj.contains("data")) {
            *type = BROADCAST;
            qm_broad* broad = static_cast<qm_broad*>(std::malloc(sizeof(qm_broad)));
