...
mqd_t init_queue(char *queue)
{
    ...
    mq = mq_open(queue, O_CREAT | O_RDWR /*| O_RDONLY | O_NONBLOCK*/, 0777, &attr); //TODO: Better define permissions
    printf("mqopen %d\n", mq);
    if (mq == (mqd_t)-1) {
        print_err("mq_open cannot create que in %s %d %s", queue, errno, strerror(errno));
        print_msg("mq_open cannot create que in %s %d %s", queue, errno, strerror(errno));
        return 0;
    }
